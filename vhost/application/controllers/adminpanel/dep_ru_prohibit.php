...
class Dep_ru_prohibit extends Admin_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model("Dep_ru_prohibit_model");
    }
    ...
    public function add()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $new_id = $this->Dep_ru_prohibit_model->insert($arr);
            if ($new_id) {
                exit(json_encode(array(
                    'status' => true,
                    'tips' => '信息新增成功',
            ...
            $data = $this->Tablelist_model->gettypebyname("tb_dep_ru_prohibit");
            if ($data)
    ...
    public function modify($id)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $new_id = $this->Dep_ru_prohibit_model->update($arr, ' ru_id = ' . $id);
            if ($new_id) {
                exit(json_encode(array(
                    'status' => true,
                    'tips' => '信息新增成功',
            ...
            $data = $this->Dep_ru_prohibit_model->get_one(array(
                'ru_id' => intval($id)
            ));
            if (isset($data)) {
                ...
                $this->load->model('Tablelist_model');
                $datat = $this->Tablelist_model->gettypebyname("tb_dep_ru_prohibit");
                if ($datat)
    ...
    {
        $data_info = $this->Dep_ru_prohibit_model->get_one(array(
            'ru_id' => $id
        ));
        ...
        $status = $this->Dep_ru_prohibit_model->delete(array(
            'ru_id' => $id
        ));
