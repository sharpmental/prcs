	...
	public function viewtable($id='1', $start='0'){
		...
		if ($data->result_array()){
			foreach ( $data->result_array () as $k => $v ) {
				array_push($v, '<a class="btn btn-info btn-sm" href="'.current_url().$v[reset($fields)].'" role="button">修改</a>');
				array_push($v, '<a class="btn btn-info btn-sm" href="'.current_url().$v[reset($fields)].'" role="button">删除</a>');
				array_push ( $data_t, $v );
			}
		}
		else {
